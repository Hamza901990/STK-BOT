...
function buildModerationLogData(event) {
  ...
  if (event.metadata) {
    Object.entries(event.metadata).forEach(([key, value]) => {
      if (value !== undefined && value !== null && key !== 'userId' && key !== 'moderatorId' && key !== 'avatarURL') {
  ...
  return {
    ...
    thumbnail: event.metadata?.avatarURL
      || (targetId ? `https://cdn.discordapp.com/embed/avatars/${Number(targetId) % 5}.png` : undefined),
